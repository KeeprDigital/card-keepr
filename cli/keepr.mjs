...
import { runReleaseApproveCommand } from "./release-approve.mjs";
...
const commandRoutes = {
  ...
  ...Object.fromEntries(
    ["pause", "resume", "abandon"].map((action) => [
      `gameCandidate${action[0].toUpperCase()}${action.slice(1)}`,
      {
        path: `/v1/game-candidates/{candidate-id}/${action}`,
        fields: { generation: "generation", idempotency_key: "idempotency-key" },
        integers: ["generation"],
        yes: true,
      },
...
async function routeCommand(name, arguments_, environment, json, { timeoutMs } = {}) {
  ...
  if (definition.production && value("environment") !== (environment.KEEPR_TARGET ?? "production"))
    return productionTargetFailure(json, targetConfirmationDetail(definition.production, environment));
  const mapped = (fields) =>
    Object.fromEntries(
...
const commands = {
  "docs catalogue": (args, env, json) => runDocumentationCommand("catalogue", args, env, json),
  "docs administration": (args, env, json) => runDocumentationCommand("administration", args, env, json),
  ...
  "release approve": runReleaseApproveCommand,
  "release production": runProductionReleaseCommand,
  "release run": runReleaseRunCommand,
  ...
  "game-candidate pause": (args, env, json) => routeCommand("gameCandidatePause", args, env, json),
  "game-candidate resume": (args, env, json) => routeCommand("gameCandidateResume", args, env, json),
...
async function catalogueExportDeletion(action, arguments_, environment, json) {
  ...
  if (action === "confirm") {
    ...
    if (values["--environment"] !== (environment.KEEPR_TARGET ?? "production")) {
      return productionTargetFailure(json, targetConfirmationDetail("Catalogue Export deletion", environment));
    }
  ...
  if (action === "retry") {
    ...
    if (options.error !== null || required.some((name) => values[name] === undefined) || !options.flags.has("--yes"))
      return usageFailure(json);
    if (values["--environment"] !== (environment.KEEPR_TARGET ?? "production")) {
      return productionTargetFailure(json, targetConfirmationDetail("Catalogue Export deletion retry", environment));
    }
    const resolved = await resolveProductionStatus(environment, json, values["--expected-current-revision"]);
    if (typeof resolved === "number") return resolved;
    const confirmation = {
