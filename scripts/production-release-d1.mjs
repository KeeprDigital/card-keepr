...
export async function executeSqlFile(environment, { configPath, sqlPath }, fetchImpl = fetch) {
  ...
  let document;
  try { document = await response.json(); } catch { throw new Error("d1_malformed_response"); }
  ...
  for (const entry of document.result) {
    if (!record(entry) || entry.success !== true || !Array.isArray(entry.results)) throw new Error("d1_statement_failed");
  }
...
async function catalogueDatabaseId(configPath) {
  let config;
  try { config = JSON.parse(await readFile(configPath, "utf8")); } catch { throw new Error(`invalid_wrangler_config:${configPath}`); }
  const binding = (Array.isArray(config?.d1_databases) ? config.d1_databases : []).find((entry) => record(entry) && entry.binding === "CATALOGUE_DB");
  if (!binding || typeof binding.database_id !== "string" || binding.database_id.length === 0) throw new Error("missing_catalogue_database_binding");
...
function parseArguments(argv) {
  ...
  return "--config" in values && "--file" in values ? { configPath: values["--config"], sqlPath: values["--file"] } : null;
}

if (import.meta.url === `file://${process.argv[1]}`) {
  const parsed = parseArguments(process.argv.slice(2));
  if (parsed === null) {
    process.stderr.write("usage: production-release-d1.mjs execute --config <wrangler.jsonc> --file <statements.sql>\n");
    process.exit(2);
  }
  try {
