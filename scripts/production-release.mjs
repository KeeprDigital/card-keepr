...
export async function validateDispatchAndWriteSql(environment, directory) {
  ...
  const noneOr = (name, parse) => {
    ...
    if (bootstrap) { if (value !== "none") throw new Error(`bootstrap_${name.toLowerCase()}_not_none`); return null; }
    return parse(value);
  };
  const plan = {
    bootstrap,
  ...
  // The pre-migration fence is an ingestion run named after the Production
  // Release; it predates Bootstrap Mode and is held in both modes.
  const fenceRun = `release-bootstrap|${expires}|${plan.release_id}`;
  ...
  const expectedRetention = bootstrap ? "" : plan.retained_revision_evidence.map((item) => `(${q(item.revision_id)},${item.depth})`).join(",");
  ...
  const claim = replacement === null
    ? `INSERT OR IGNORE INTO ingestion_runs (id,state,selected_games_json,started_at,expected_current_revision_id,idempotency_key,candidate_json) SELECT ${q(fenceRun)},'planning','[]',strftime('%Y-%m-%dT%H:%M:%fZ','now'),${q(plan.expected_current_revision_id)},${q(fenceRun)},'{"production_release_bootstrap":true}' WHERE ${claimedEvidence} AND ${liveGate}; UPDATE operation_state SET active_ingestion_run_id=${q(fenceRun)} WHERE singleton=1 AND active_ingestion_run_id IS NULL AND recovery_health='healthy' AND ${claimedEvidence} AND ${liveGate};`
  ...
  await writeFile(`${directory}/claim.sql`, `${durableClaim}\n${claim}\nSELECT changes() AS changed_rows, CASE WHEN ${replacement === null ? `active_ingestion_run_id=${q(fenceRun)}` : `active_release_id=${q(plan.release_id)}`} AND ${claimedEvidence} THEN 1 ELSE 0 END AS claimed FROM operation_state WHERE singleton=1;\n`, { mode: 0o600 });
...
export async function writeReplacementSeedSql(environment, serializedEvidence, output) {
  ...
  statements.push(
    ...
    `UPDATE operation_state SET active_release_id=${q(productionRelease.id)},active_release_expires_at=${q(evidence.operation_state.active_production_release_expires_at)} WHERE singleton=1 AND active_ingestion_run_id IS NULL AND recovery_health='blocked' AND recovery_restore_guard='blocked' AND active_recovery_id=${q(plan.replacement_handoff.recovery_id)} AND (active_release_id IS NULL OR (active_release_id=${q(productionRelease.id)} AND active_release_expires_at=${q(evidence.operation_state.active_production_release_expires_at)}));`,
...
export async function writeEvidenceSql(kind, releaseId, evidence, output, environment = {}) {
  ...
  if (bootstrap) {
    await writeFile(output, `${bootstrapEvidenceSql(kind, id, serialized, digest(required(environment, "DISPATCH_DIGEST")))}\n`, { mode: 0o600 });
    return;
  }
...
function bootstrapEvidenceSql(kind, id, serialized, dispatchDigest) {
  ...
  const rowExists = (key, operation) => `EXISTS (SELECT 1 FROM administration_idempotency WHERE idempotency_key=${q(key)} AND operation=${q(operation)})`;
  const leaseHeld = `EXISTS (SELECT 1 FROM operation_state WHERE singleton=1 AND active_release_id=${q(id)})`;
  ...
  if (kind === "binding") {
    return ledgerInsert(bindingKey, "production_release_binding_observed", deployingKey, "production_release_deploying", stableJson({ release_id: id, state: "smoke_testing", dispatch_digest: dispatchDigest, binding_observation: JSON.parse(serialized) }));
  }
  return `${ledgerInsert(smokeKey, "production_release_succeeded", bindingKey, "production_release_binding_observed", stableJson({ release_id: id, state: "succeeded", dispatch_digest: dispatchDigest, smoke_evidence: JSON.parse(serialized) }))} UPDATE operation_state SET active_release_id=NULL,active_release_expires_at=NULL WHERE singleton=1 AND active_release_id=${q(id)} AND ${rowExists(smokeKey, "production_release_succeeded")}; SELECT changes() AS changed_rows, CASE WHEN changes()=1 AND EXISTS (SELECT 1 FROM operation_state WHERE singleton=1 AND active_release_id IS NULL AND active_ingestion_run_id IS NULL) THEN 1 ELSE 0 END AS fence_released;`;
}

function booleanInput(value) { if (value !== "true" && value !== "false") throw new Error("invalid_bootstrap_input"); return value === "true"; }
...
function validReleaseEvidence(plan) {
  if (plan.bootstrap) {
    return plan.expected_current_revision_id === SPINE_REVISION_ID && plan.replacement_handoff === null &&
      plan.recovery_bookmark === null && plan.recovery_backup_attempt_id === null &&
      plan.retained_revision_evidence === null && plan.smoke_targets === null;
  }
...
SELECT json_object(
  ...
  'operation_state',json_object('active_ingestion_run_id',operation.active_ingestion_run_id,'active_production_release_id',operation.active_release_id,'active_production_release_expires_at',operation.active_release_expires_at,'recovery_health',operation.recovery_health,'active_recovery_id',operation.active_recovery_id,'recovery_restore_guard',operation.recovery_restore_guard),
...
function validReplacementHandoffEvidence(evidence, plan, environment) {
  if (!exactKeys(evidence, ["backups", "contract", "idempotency", "operation_state", "production_release", "recoveries", "schema_migration_level"]) ||
      ...
      !exactKeys(plan, ["bootstrap", "expected_actor", "expected_current_revision_id", "expected_head_sha", "expected_migration_level", "idempotency_key", "production_target", "production_target_digest", "recovery_backup_attempt_id", "recovery_bookmark", "release_id", "replacement_handoff", "retained_revision_evidence", "smoke_targets"]) ||
  ...
  if (!exactKeys(operation, ["active_ingestion_run_id", "active_recovery_id", "active_production_release_expires_at", "active_production_release_id", "recovery_health", "recovery_restore_guard"]) ||
      operation.active_ingestion_run_id !== null || operation.active_production_release_id !== plan.release_id ||
      operation.active_recovery_id !== plan.replacement_handoff.recovery_id || operation.recovery_health !== "blocked" ||
      operation.recovery_restore_guard !== "blocked" || !isoTimestamp(operation.active_production_release_expires_at)) return false;
...
if (process.argv[2] === "replacement-configs") {
  ...
  await writeEvidenceSql(process.argv[3], process.argv[4], process.env.KEEPR_RELEASE_EVIDENCE, process.argv[5], process.env);
}
