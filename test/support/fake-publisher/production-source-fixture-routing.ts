export type ProductionSourceFixtureRole = "retained-discovery" | "surface";

const requestRoleSuffix = /;\s*request-role=(?:surface|listing|detail|product_detail|image)(?=;|$)/gu;
const requestSurfaceSuffix = /;\s*request-surface=[a-z0-9]+(?:-[a-z0-9]+)*(?=;|$)/gu;

export function productionSourceFixtureRole(headers: Headers): ProductionSourceFixtureRole {
  const role = headers
    .get("user-agent")
    ?.match(/(?:^|;\s*)request-role=(surface|listing|detail|product_detail|image)(?:;|$)/u)?.[1];
  return role === undefined ? "retained-discovery" : "surface";
}

export function productionSourceFixtureIsProductDetail(headers: Headers): boolean {
  return /(?:^|;\s*)request-role=product_detail(?:;|$)/u.test(headers.get("user-agent") ?? "");
}

export function productionSourceFixtureMarker(headers: Headers): string | null {
  return headers.get("user-agent")?.replace(requestRoleSuffix, "").replace(requestSurfaceSuffix, "") ?? null;
}

export function productionSourceFixtureSurface(headers: Headers): string | null {
  return headers.get("user-agent")?.match(/(?:^|;\s*)request-surface=([a-z0-9]+(?:-[a-z0-9]+)*)(?:;|$)/u)?.[1] ?? null;
}
...
function productionFusionCardSearchLeaf(url: URL, officialNavigation: string): Response | null {
  ...
  if (
    ...
    [...url.searchParams.keys()].some((key) => key !== "search" && key !== "category[0]")
  )
    return null;
  return new Response(
...
const officialProductTitleSuffix: Record<string, string> = {
  ...
  "fusion-world-en": " | Dragon Ball Super Card Game Fusion World - Official Web Site",
...
function productionOfficialProductDetailResponse(lineage: string, url: URL): Response {
  const suffix = officialProductTitleSuffix[lineage] ?? "";
  return new Response(
    `<html><title>${officialProductStageTitle}${suffix}</title>
      <h1>BANDAI CARD GAMES</h1>${
        lineage.startsWith("gundam-") ? `<h2 class="mvColTitle">${officialProductStageTitle}</h2>` : ""
      }
...
export function productionOfficialStageResponse(
  ...
  if (lineage === "fusion-world-en") {
    const cardSearchLeaf = productionFusionCardSearchLeaf(url, officialNavigation);
  ...
  return new Response(
    `<html><title>Official Bandai CARD PRODUCT RELEASE RULE ERRATA RESTRICTION publication</title>${officialNavigation}${productionOfficialStageNavigation(lineage, url)}<main>${
      ...
      (url.hostname === "www.dbs-cardgame.com" && url.pathname === "/fw/en/rules/errata-card/") ||
      (url.hostname === "world.digimoncard.com" && url.pathname === "/rule/restriction_card/") ||
      (
        url.hostname === "en.onepiece-cardgame.com" &&
