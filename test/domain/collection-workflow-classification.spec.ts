...
test("a dead Workflow classifies by its known status, never by elapsed time", () => {
  expect(classifyCollectionWorkflow(facts({ workflow_status: "errored" }))).toEqual({
    kind: "recover",
    reason: "source_workflow_errored",
  });
  expect(classifyCollectionWorkflow(facts({ workflow_status: "terminated" }))).toEqual({
    kind: "recover",
    reason: "source_workflow_terminated",
  });
  expect(classifyCollectionWorkflow(facts({ workflow_status: "unknown" }))).toEqual({
    kind: "recover",
    reason: "source_workflow_unavailable",
  });
  expect(classifyCollectionWorkflow(facts({ workflow_status: "unavailable" }))).toEqual({
    kind: "recover",
    reason: "source_workflow_unavailable",
  });
});
...
test("a paused Workflow instance resumes rather than pausing the run", () => {
  expect(classifyCollectionWorkflow(facts({ workflow_status: "paused" }))).toEqual({ kind: "instance_paused" });
});
...
test("recent persisted progress keeps a running Workflow active", () => {
  ...
  expect(classifyCollectionWorkflow(facts({ workflow_status: "queued" }))).toEqual({ kind: "active" });
  expect(classifyCollectionWorkflow(facts({ workflow_status: "waiting" }))).toEqual({ kind: "active" });
});
...
test("a run with no persisted progress evidence at all is stalled", () => {
  expect(classifyCollectionWorkflow(facts({ last_progress_ms: null }))).toEqual({
    kind: "recover",
...
test("a lost instance is distinguished from a transient control-plane error", () => {
  ...
  expect(isWorkflowInstanceNotFound(new Error("Workflow instance not found"))).toBe(true);
...
test("attempt records derive their scope from the instance identity", () => {
  ...
  expect(workflowAttemptRecord("run_1", "evidence-host-abc123-attempt-1")).toEqual({
    workflow_kind: "child",
    base_workflow_id: "evidence-host-abc123",
    attempt_number: 3,
