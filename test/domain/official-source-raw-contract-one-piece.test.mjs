...
test("retained live One Piece policy bytes publish the complete current active list", () => {
  ...
  const rules = retainedLegalityRules(adapter, "restrictions", "one-piece-en-policy");
  assert.deepEqual(
    rules.map((rule) => ({
...
test("the issue-58 One Piece don-rules contract retains the live hub as coverage without DON payload", () => {
  ...
  assert.equal(retained.document_title, "RULES｜ONE PIECE CARD GAME - Official Web Site");
  for (const pinned of [
...
test("retained One Piece and Digimon policies reject unconsumed event scope while allowing text-free framing", () => {
  const cases = [
    ...
    },
    {
      ...
      anchor: '<h4 class="subTit txtNormal">List of Currently Affected Cards</h4>',
    },
  ];
  for (const descriptor of cases) {
    ...
    const parse = (html) =>
      adapter.parseBytes(Buffer.from(html), {
        mediaType: fixture.metadata.content_type,
        url: fixture.metadata.source_url,
    ...
    assert.doesNotThrow(() =>
      parse(source.replace(descriptor.anchor, `<div class="publisher-frame"></div>${descriptor.anchor}`)),
    );
  }
});
...
test("the versioned One Piece release surface emits its exact release-timing rule beside Release evidence", () => {
  ...
  payload.release_timing_entries = [
    {
      ...
      published_text: "OP99-001 becomes legal for standard tournament play on 2026-09-04.",
  ...
  assert.ok(
    observations.some(({ product_release_catalogue }) =>
      product_release_catalogue?.products?.some(({ releases }) => releases.length > 0),
    ),
  );
  ...
  assert.deepEqual(legality.legality_rules, [
    {
      ...
      card_numbers: ["OP99-001"],
      official_wording: "OP99-001 becomes legal for standard tournament play on 2026-09-04.",
...
test("the expanded One Piece adapter emits exact typed Errata and rejects unrepresentable entries", () => {
  ...
  const observations = adapter.parseBytes(
    new TextEncoder().encode(`<html>${officialPublisherPayloadScript("one-piece-en", "errata", payload)}</html>`),
    {
      mediaType: "text/html; charset=utf-8",
...
test("the expanded One Piece adapter closes the nested DON Card policy schema", () => {
  ...
  assert.throws(
    () =>
      adapter.parseBytes(
        new TextEncoder().encode(
          `<html>${officialPublisherPayloadScript("one-piece-en", "don-rules", payload)}</html>`,
        ),
        {
          mediaType: "text/html; charset=utf-8",
...
test("the retained One Piece restriction publication is proven at its live redirect target", () => {
  ...
  assert.equal(fixture.metadata.source_url, "https://en.onepiece-cardgame.com/rules/restriction/");
  const observations = adapter.parseBytes(fixture.bytes, {
    mediaType: fixture.metadata.content_type,
    url,
    requestId: "one-piece-en:restrictions",
  });
  const legality = observations.find((observation) => observation.observation_type === "legality_rules");
  assert.deepEqual(
  ...
  const surface = observations.find((observation) => observation.observation_type === undefined).source_sidecar.raw
    .official_surfaces[0];
...
test("the One Piece Block Number publication is an exactly empty policy surface", () => {
  ...
  const fixture = retainedOfficialSourceFixture("one-piece-en-block-policy-topic");
