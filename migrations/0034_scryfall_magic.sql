-- #326: widen current closed game contracts; preserve all retained rows and fences.

SELECT CASE WHEN (SELECT migration_level FROM catalogue_schema_state WHERE singleton=1)=33
THEN 1 ELSE json_extract('schema_level_mismatch_expected_33','$') END;
...
CREATE TABLE reconciled_errata_magic AS SELECT "id","game","target_type","target_id","effective_from","official_wording","corrected_value_json","first_revision_id","last_observed_revision_id" FROM reconciled_errata;
...
CREATE TABLE reconciled_errata (
  ...
  game TEXT NOT NULL CHECK (
    game IN ('one-piece', 'fusion-world', 'digimon', 'gundam', 'riftbound', 'magic')
  ),
...
INSERT INTO source_freshness("game","area","source_lineage","region","checked_at","ingestion_run_id") SELECT "game","area","source_lineage","region","checked_at","ingestion_run_id" FROM source_freshness_magic;

DROP TABLE source_freshness_magic;

CREATE TABLE curated_revisions_magic AS SELECT "id","game","target_key","target_kind","effective_from","effective_to","proposal_json","content_digest","reviewed_source_digest","schema_binding_json","author","created_at","status","event_version" FROM curated_revisions;
...
INSERT INTO reconciliation_checkpoints("preparation_id","phase","ordinal","content","sha256") SELECT "preparation_id","phase","ordinal","content","sha256" FROM reconciliation_checkpoints_magic;

DROP TABLE reconciliation_checkpoints_magic;

-- Retain the ON DELETE CASCADE child while its parent is rebuilt.
CREATE TABLE revision_printing_product_query_magic AS SELECT * FROM revision_printing_product_query;

CREATE TABLE revision_printing_query_magic AS SELECT "catalogue_revision_id","printing_id","card_id","supported_game","normalized_rarity" FROM revision_printing_query;
...
CREATE TABLE revision_printing_query (
  ...
  supported_game TEXT NOT NULL CHECK (supported_game IN ('one-piece', 'fusion-world', 'digimon', 'gundam', 'riftbound', 'magic')),
...
INSERT INTO revision_printing_query("catalogue_revision_id","printing_id","card_id","supported_game","normalized_rarity") SELECT "catalogue_revision_id","printing_id","card_id","supported_game","normalized_rarity" FROM revision_printing_query_magic;

DROP TABLE revision_printing_query_magic;

INSERT INTO revision_printing_product_query SELECT * FROM revision_printing_product_query_magic;

DROP TABLE revision_printing_product_query_magic;

CREATE TABLE ingestion_run_selected_games_magic AS SELECT "ingestion_run_id","ordinal","game" FROM ingestion_run_selected_games;
...
INSERT INTO ingestion_run_selected_games("ingestion_run_id","ordinal","game") SELECT "ingestion_run_id","ordinal","game" FROM ingestion_run_selected_games_magic;

DROP TABLE ingestion_run_selected_games_magic;
...
-- Independent Source registration grants neither admission nor publication.
...
VALUES ('scryfall-magic-en@1','scryfall-magic-en','magic','magic@1','scryfall-magic-card-pilot@1','production',10);

INSERT INTO game_catalogue_heads(supported_game,revision_id) VALUES ('magic','catrev_spine_000');

UPDATE catalogue_schema_state SET migration_level=34 WHERE singleton=1;
