-- #328: widen current closed game contracts; preserve all retained rows and fences.

SELECT CASE WHEN (SELECT migration_level FROM catalogue_schema_state WHERE singleton=1)=34
THEN 1 ELSE json_extract('schema_level_mismatch_expected_34','$') END;
...
CREATE TABLE reconciled_errata_pokemon AS SELECT "id","game","target_type","target_id","effective_from","official_wording","corrected_value_json","first_revision_id","last_observed_revision_id" FROM reconciled_errata;
...
CREATE TABLE reconciled_errata (
  ...
  game TEXT NOT NULL CHECK (
    game IN ('one-piece', 'fusion-world', 'digimon', 'gundam', 'riftbound', 'magic', 'pokemon')
  ),
...
INSERT INTO source_freshness("game","area","source_lineage","region","checked_at","ingestion_run_id") SELECT "game","area","source_lineage","region","checked_at","ingestion_run_id" FROM source_freshness_pokemon;

DROP TABLE source_freshness_pokemon;

CREATE TABLE curated_revisions_pokemon AS SELECT "id","game","target_key","target_kind","effective_from","effective_to","proposal_json","content_digest","reviewed_source_digest","schema_binding_json","author","created_at","status","event_version" FROM curated_revisions;
...
INSERT INTO reconciliation_checkpoints("preparation_id","phase","ordinal","content","sha256") SELECT "preparation_id","phase","ordinal","content","sha256" FROM reconciliation_checkpoints_pokemon;

DROP TABLE reconciliation_checkpoints_pokemon;

-- Retain the ON DELETE CASCADE child while its parent is rebuilt.
CREATE TABLE revision_printing_product_query_pokemon AS SELECT * FROM revision_printing_product_query;

CREATE TABLE revision_printing_query_pokemon AS SELECT "catalogue_revision_id","printing_id","card_id","supported_game","normalized_rarity" FROM revision_printing_query;
...
CREATE TABLE revision_printing_query (
  ...
  supported_game TEXT NOT NULL CHECK (supported_game IN ('one-piece', 'fusion-world', 'digimon', 'gundam', 'riftbound', 'magic', 'pokemon')),
...
INSERT INTO revision_printing_query("catalogue_revision_id","printing_id","card_id","supported_game","normalized_rarity") SELECT "catalogue_revision_id","printing_id","card_id","supported_game","normalized_rarity" FROM revision_printing_query_pokemon;

DROP TABLE revision_printing_query_pokemon;

INSERT INTO revision_printing_product_query SELECT * FROM revision_printing_product_query_pokemon;

DROP TABLE revision_printing_product_query_pokemon;

CREATE TABLE ingestion_run_selected_games_pokemon AS SELECT "ingestion_run_id","ordinal","game" FROM ingestion_run_selected_games;
...
INSERT INTO ingestion_run_selected_games("ingestion_run_id","ordinal","game") SELECT "ingestion_run_id","ordinal","game" FROM ingestion_run_selected_games_pokemon;

DROP TABLE ingestion_run_selected_games_pokemon;
...
-- Registration remains separate from qualification, owner admission and publication.
...
VALUES ('tcgdex-pokemon-en@1','tcgdex-pokemon-en','pokemon','pokemon@1','tcgdex-pokemon-rest-card@1','production',4),
       ('pokemon-official-en@1','pokemon-official-en','pokemon','pokemon@1','pokemon-selected-publications-html@1','production',4);

INSERT INTO game_catalogue_heads(supported_game,revision_id) VALUES ('pokemon','catrev_spine_000');

UPDATE catalogue_schema_state SET migration_level=35 WHERE singleton=1;
