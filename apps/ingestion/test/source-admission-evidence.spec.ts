...
test("archive review intake survives two committed response losses and keeps terminal evidence and finish decisions separate", async () => {
  const { db, run, snapshot, request } = await seedArchive(
    "review-archive-replay",
    false,
    new TextEncoder().encode([bloomvine, control].map((raw) => (raw.endsWith("\n") ? raw : `${raw}\n`)).join("")),
  );
  let parseLoss = false,
    proposalLoss = false;
  ...
  expect(review.source_sidecar.source_record_json).toBe(JSON.stringify(JSON.parse(bloomvine)));
  ...
  expect(captured).toMatchObject({ processed: 3, halt: null });
  ...
  for (const id of resumed.proposalIds) {
    ...
    expect((await queries.reviewPrivateReferences(db).bind("entity_proposal", id).all()).results).toHaveLength(2);
  }
