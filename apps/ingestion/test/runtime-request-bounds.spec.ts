...
test("final Official Source requests keep discovery evidence immutable while exposing a deterministic fixture role", async () => {
  ...
  expect(
    ...
    requests.map(({ surface }) => ({
      ...
      headers: {
        accept: "text/html",
        "user-agent": `card-keepr-representable-legality-v3; request-role=surface; request-surface=${surface}`,
      },
    })),
  );
});
...
test("the exact live Fusion final request selects and parses the representable legality fixture", async () => {
  ...
  const current = requests.find(({ surface }) => surface === "legality-current");
  if (current === undefined) throw new Error("current Legality request missing");
  expect(current.url).toBe("https://www.dbs-cardgame.com/fw/en/news/01_305.html");
  ...
  const legality = observations?.find(
    (observation) =>
      ...
      observation !== null &&
      (observation as Record<string, unknown>).observation_type === "legality_rules",
  ...
  expect(
    productionRepresentableFusionLegalityResponse(
      new Request("https://www.dbs-cardgame.com/fw/en/rules/banned-limited-cards/", { headers: current.headers }),
    ),
  ).toBeNull();
});
...
test("final Official Source collection identities enforce the merged-header byte bound", async () => {
  ...
  const headerBase = utf8(
    canonicalJson({
      accept: "text/html",
      "user-agent": "card-keepr-official-source/1; request-role=surface; request-surface=legality-current",
      "x-final-bound": "",
    }),
  ).byteLength;
  await expect(
