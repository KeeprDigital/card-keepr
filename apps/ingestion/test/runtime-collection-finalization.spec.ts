...
import { createCollection, installRuntimeSuite, resumeCollection, showCollection } from "./runtime-helpers";
...
test("finalizing an already completed collection again leaves its completion time unchanged", async () => {
  const run = await createCollection("finalization_idempotent_001", "https://official-source.invalid/cards");
  ...
  expect(again.collection?.collection_completed_at).toBe(completed.collection?.collection_completed_at);
});
