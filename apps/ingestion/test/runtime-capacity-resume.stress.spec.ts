...
test("an extended production-shaped run resumes into bounded host shards without refetching", async () => {
  const { runId, root } = await pauseRunAtCapacity("source_capacity_resume_stress_001", "observed");
  ...
  try {
    ...
    observed = await waitForEvidenceCondition(
      runId,
      (current) => current.state === "collecting" && current.workflow.child_ids.length >= 2,
      60_000,
    );
    ...
    expect(
      await sourceEvidenceQueries.readSourceRequestsState(env.CATALOGUE_DB).bind(runId, root.request_id).first("state"),
    ).toBe("observed");
    ...
    expect(
      await sourceEvidenceQueries
        .countSourceFetchAttemptsCountForExtendedProductionShapedRunResumesIntoBoundedHostShards(env.CATALOGUE_DB)
