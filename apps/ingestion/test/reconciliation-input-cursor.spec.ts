      ...
      new Proxy(statement, {
        get(target, property) {
          ...
          if (
            ...
            resumed &&
            (successfulImages === 1
              ? sql.includes("FROM source_record_pages") && values.includes(firstSet.id)
...
test.each(["r2", "receipt"])("source manifests and record pages resume after %s storage failure", async (failure) => {
      ...
      if (property === "get")
        return (...args: Parameters<R2Bucket["get"]>) => {
          ...
          if (args[0].startsWith("source-observations/")) {
            ...
            if (failure === "r2" && !resumed) {
              failures++;
              throw new Error("Injected source manifest storage outage.");
            }
  ...
  const database = new Proxy(testEnv.CATALOGUE_DB, {
    get(target, property) {
      if (property === "prepare")
        return (sql: string) => {
          if (failure === "receipt" && completedBytes > 0 && !resumed && sql.includes("FROM source_record_pages")) {
            failures++;
