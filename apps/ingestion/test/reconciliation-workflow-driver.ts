...
export async function runReconciliationWorkflow(
  ...
  step: WorkflowStep,
  trace?: { callbacks: Map<string, number>; created: string[]; loseCreateResponse?: boolean },
) {
  ...
  function instance(id: string) {
    ...
    return {
      id,
      status: async () => ({ status: states.get(id)! }),
      sendEvent: async (message: { type: string; payload: unknown }) => {
  ...
  const binding = new Proxy(env.RECONCILIATION_WORKFLOW, {
    get(target, property) {
      if (property === "get") return async (id: string) => instance(id);
      if (property === "create")
