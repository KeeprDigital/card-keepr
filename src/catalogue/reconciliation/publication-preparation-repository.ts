...
export function publicationPreparationGuard(
  ...
  return repositoryStatements(db)
    ...
    WHEN NOT EXISTS (SELECT 1 FROM game_candidates c JOIN reconciliation_operations o ON o.id=c.preparation_id
      WHERE c.id=?1 AND json_extract(o.definition_pins_json,'$.card_model')='categories')
      THEN json_extract('{}','reconciliation_definition_changed')
    WHEN NOT EXISTS (SELECT 1 FROM game_candidates c JOIN reconciliation_operations o ON o.id = c.preparation_id
...
export function retainPublicReadFacts(
 ...
 CASE WHEN b.kind='cards' THEN json_extract(b.content,'$.records[0].value.category') ELSE NULL END
