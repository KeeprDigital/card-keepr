...
export function priorPublicLifecycle(db: CatalogueStore, revision: string, game: string, kind: string, id: string) {
  return repositoryStatements(db)
    .prepare(`SELECT l.* FROM catalogue_composition_games m
 JOIN publication_read_lifecycles l ON l.candidate_id=m.candidate_id
 WHERE m.catalogue_revision_id=? AND m.supported_game=? AND l.kind=? AND l.entity_id=?`)
    .bind(revision, game, kind, id);
}
export function publicationObservedPlan(
 ...
 MAX(NOT EXISTS(SELECT 1 FROM catalogue_composition_games m JOIN game_candidates prior ON prior.id=m.candidate_id
 ...
 WHERE m.catalogue_revision_id=?2 AND m.supported_game=?3)) OVER() AS newly_observed
