...
export function retainNormalizedObservationStatement(
  ...
  return repositoryStatements(database)
    .prepare(`INSERT INTO reconciliation_normalized_observations (ingestion_run_id, observation_id, content, sha256)
    VALUES (?, ?, ?, ?) ON CONFLICT (ingestion_run_id, observation_id) DO NOTHING`)
    .bind(runId, id, content, sha256);
}

export function normalizedObservationStatement(database: CatalogueStore, runId: string, id: string) {
  return repositoryStatements(database)
    .prepare(
      `SELECT content, sha256 FROM reconciliation_normalized_observations WHERE ingestion_run_id = ? AND observation_id = ?`,
    )
    .bind(runId, id);
}
