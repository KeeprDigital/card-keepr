...
export function normalizeOnePieceCardPage(value: Readonly<Record<string, unknown>>): {
  attributes: Record<string, unknown>;
  printingAttributes?: Record<string, unknown>;
  normalizedRarity: string | null;
} {
  const cardType = controlledValue(value.Category, cardTypes, "One Piece Category");
  ...
  const normalizedIllustrationTypes =
    rawIllustrationTypes === undefined
      ? []
      : uniqueValues(rawIllustrationTypes, "One Piece illustration filter membership")
  ...
  return {
    normalizedRarity: printing === null ? null : normalizedOnePieceRarity(printing.rarity),
    attributes: {
      card_type: cardType,
      colours,
      cost,
      life,
      battle_attributes: uniqueValues(value.Attribute, "One Piece Attribute").map(normalizedToken).sort(),
      ...
      traits: uniqueValues(value.Type, "One Piece Type").map(normalizedText).sort(),
      block_icons: uniqueValues(value["Block icon"], "One Piece Block icon").map(normalizedText).sort(),
      effect_text: nullableText(value.Effect, "One Piece Effect"),
      trigger_text: nullableText(value.Trigger, "One Piece Trigger"),
    },
    printingAttributes:
      normalizedIllustrationTypes.length === 0 ? undefined : { illustration_types: normalizedIllustrationTypes },
  };
}
...
export function onePieceDonCardObservation(value: unknown): Record<string, unknown> {
  ...
  return {
    ...
    source_sidecar: {
      raw: { don_card: card },
      consumed_fields: ["don_card.Category", "don_card.Effect", "don_card.functional_designation", "don_card.name"],
...
export function onePieceRecordingMemberships(value: unknown): ReadonlyMap<string, readonly string[]> {
  ...
  return new Map([...memberships].map(([locator, recordings]) => [locator, [...recordings].sort()]));
}

function controlledValue(value: unknown, vocabulary: ReadonlyMap<string, string>, name: string): string {
