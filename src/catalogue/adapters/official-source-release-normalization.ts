...
export function normalizedOfficialReleaseDate(value: string, options: { seasons?: boolean } = {}): OfficialReleaseDate {
  ...
  const quarter = normalized.match(/^(?:Q([1-4])\s+(\d{4})|(\d{4})[\s-]+Q([1-4]))$/iu);
  if (quarter !== null) {
  ...
  const monthFirst = normalized.match(/^([A-Za-z]+)\s+(\d{1,2}),?\s+(\d{4})$/u);
  const dayFirst = normalized.match(/^(\d{1,2})\s+([A-Za-z]+)\s+(\d{4})$/u);
...
export function normalizedOfficialReleaseStatus(value: string | null): "announced" | "released" | null {
  ...
  if (/^(?:released|on sale|available|available now|now available|sales? start(?:ed)?)$/u.test(normalized)) {
    return "released";
  }
...
export function officialReleaseStatusNeedsSchemaReview(value: string | null): boolean {
  return unavailableOfficialReleaseVocabulary(value);
}
...
function unavailableOfficialReleaseVocabulary(value: string | null): boolean {
  return (
    value !== null &&
    /^(?:|-|tba|tbd|to be announced|to be determined|unknown|not announced)$/iu.test(value.normalize("NFC").trim())
  );
}
