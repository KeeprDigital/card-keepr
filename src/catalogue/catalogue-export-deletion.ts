...
export async function prepareCatalogueExportDeletion(
  ...
  const priorPlan = await database.prepare(
    "SELECT 1 AS present FROM catalogue_export_deletion_plans WHERE id = ?",
  ).bind(request.plan_id).first();
  if (priorPlan !== null) {
...
export async function confirmCatalogueExportDeletion(
  ...
  const replay = await database.prepare(
    "SELECT * FROM catalogue_export_deletions WHERE idempotency_key = ?",
  ).bind(request.idempotency_key).first<DeletionRow>();
  if (replay !== null) {
  ...
  const catalogueExport = await database.prepare(
    `SELECT catalogue_revision_id, manifest_key, manifest_digest, maintenance_state
     FROM catalogue_exports WHERE catalogue_revision_id = ?`,
  ).bind(plan.catalogue_revision_id).first<ExportRow>();
  if (
  ...
  const existingIdentity = await database.prepare(
    "SELECT 1 AS present FROM catalogue_export_deletions WHERE id = ?",
  ).bind(request.deletion_id).first();
...
export async function retryCatalogueExportDeletion(
  ...
  if (
    replay !== null &&
    row.retry_owner_idempotency_key !== request.idempotency_key
  ) {
    throw problem(
      409,
      "export_deletion_not_failed",
      "Another retry owns the active deletion attempt.",
    );
  }
  ...
  if (replay !== null) {
    ...
    if (!claimed) {
      return waitForDeletionResponse(
        database,
        deletionId,
        request.idempotency_key,
      );
  ...
  if (replay === null) {
    try {
      ...
      throw problem(
        409,
        "export_deletion_not_failed",
        "Another retry claimed the failed deletion.",
      );
    }
  }
  const manifestKey = await database.prepare(
    "SELECT manifest_key FROM catalogue_exports WHERE catalogue_revision_id = ?",
  ).bind(plan.catalogue_revision_id).first<string>("manifest_key");
...
async function executeDeletion(
  ...
  try {
    ...
    await database.batch([
      ...
      const reconciled = await loadDeletionResponse(
        database,
        deletionId,
        retryIdempotencyKey,
      );
...
function leaseExpiresAt(observedAt: string): string {
  return new Date(
    new Date(observedAt).valueOf() + EXECUTION_LEASE_MS,
  ).toISOString();
}
...
async function loadDeletionResponse(
  ...
  const responseJson = retryIdempotencyKey === null
    ? await database.prepare(
      "SELECT confirmation_response_json FROM catalogue_export_deletions WHERE id = ?",
    ).bind(deletionId).first<string>("confirmation_response_json")
    : await database.prepare(
      "SELECT response_json FROM catalogue_export_deletion_retries WHERE idempotency_key = ? AND deletion_id = ?",
    ).bind(retryIdempotencyKey, deletionId).first<string>("response_json");
  return responseJson === null
    ? null
    : JSON.parse(responseJson) as Record<string, unknown>;
}
...
async function assertMutationGuards(
  ...
  if (plan.catalogue_revision_id === plan.expected_current_revision_id) {
    throw problem(409, "current_export_required", "Publish or recover another Catalogue Revision before deleting this export.");
  }
  await assertMaintenanceIdle(database, plan.expected_current_revision_id, observedAt);
}
...
async function currentRevision(database: D1Database): Promise<string> {
  const revisionId = await database.prepare(
...
async function loadPlan(database: D1Database, planId: string): Promise<PlanRow | null> {
  return database.prepare(
    "SELECT * FROM catalogue_export_deletion_plans WHERE id = ?",
