...
export class StreamingSha256 {
  ...
  #compress(block: Uint8Array): void {
    ...
    for (let index = 16; index < 64; index += 1) {
      ...
      const sigma0 = rotateRight(first, 7) ^ rotateRight(first, 18) ^
        (first >>> 3);
      const sigma1 = rotateRight(second, 17) ^ rotateRight(second, 19) ^
        (second >>> 10);
    ...
    for (let index = 0; index < 64; index += 1) {
      const upper = rotateRight(e!, 6) ^ rotateRight(e!, 11) ^
        rotateRight(e!, 25);
      const choose = (e! & f!) ^ (~e! & g!);
      const temporary1 = (h! + upper + choose + roundConstants[index]! +
        this.#words[index]!) >>> 0;
      const lower = rotateRight(a!, 2) ^ rotateRight(a!, 13) ^
        rotateRight(a!, 22);
