// Private operational validation for retained pre-composition backup artifacts.
// These schemas are not public contracts and no consumer route serves this shape.
import Ajv2020, { type ValidateFunction } from "ajv/dist/2020.js";
import addFormats from "ajv-formats";
import manifestSchema from "./retained/retained-export-manifest.schema.json" with { type: "json" };
import recordSchema from "./retained/retained-export-record.schema.json" with { type: "json" };
...
const componentValidators = new Map<string, ValidateFunction>(
  [
    ...
    return [
      `https://card-keepr.invalid/schemas/catalogue-export-record@5#/$defs/${definition}`,
      requiredValidator(uri),
    ];
  }),
);
