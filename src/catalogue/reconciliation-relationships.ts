...
export function aggregateRelationshipEvidence(
  ...
  return [...grouped.values()]
    .map((evidence) => {
      ...
      const latest = [...(active.length > 0 ? active : evidence)].sort(
        compareLastEvidenceOrder,
      ).at(-1)!;
      return {
...
function compareFirstEvidenceOrder(
  left: RelationshipEvidenceRow,
  right: RelationshipEvidenceRow,
): number {
...
function compareLastEvidenceOrder(
  left: RelationshipEvidenceRow,
  right: RelationshipEvidenceRow,
): number {
  const leftKey = canonicalJson([
