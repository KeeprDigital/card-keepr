...
export async function attemptPublicationCleanup(
  ...
  let cleanup = await retainedPublicationCleanupStatement(database, runId).first<PublicationCleanupRow>();
  if (cleanup === null) {
  ...
  const claimed = await claimPublicationCleanupStatement(database, {
    observedAt: observedAt,
    objectKeysJson: canonicalJson(keys),
    idempotencyKey: idempotency?.key ?? null,
    requestJson: idempotency?.requestJson ?? null,
    claimToken: claimToken,
    expiresAt: claimExpiresAt,
    runId: runId,
    priorClaimVersion: cleanup.claim_version,
  ...
  try {
    ...
    const completedRow = await completePublicationCleanupStatement(database, {
      completedAt: observedAt,
      runId: runId,
      claimToken: claimToken,
      claimVersion: claimed.claim_version,
    }).first<PublicationCleanupRow>();
    ...
    await failPublicationCleanupStatement(database, {
      runId: runId,
      claimToken: claimToken,
      claimVersion: claimed.claim_version,
    }).run();
...
export async function failReservedPublication(
  ...
  const failureStatements = [
    failReservedPublicationStatement(database, { terminalAt: terminalAt, failureCode: problem.code, runId: run.id }),
    failPublicationEvidencePlanStatement(database, { failureCode: problem.code, runId: run.id }),
    releaseRunLockStatement(database, run.id),
    recordApprovalFailureStatement(database, {
      key: key,
      ...
      status: problem.status,
      createdAt: terminalAt,
      ownerToken: claimOwner?.ownerToken ?? null,
      claimVersion: claimOwner?.version ?? null,
    }),
    administrationClaimDeleteStatement(
      database,
      {
  ...
  if (objectKeys !== null) {
    failureStatements.push(
      schedulePublicationCleanupStatement(database, {
        runId: run.id,
        objectKeysJson: canonicalJson([...new Set(objectKeys)].sort()),
        notBefore: publicationCleanupNotBefore(run, terminalAt),
      }),
    );
  }
