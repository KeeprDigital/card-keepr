...
export function operationalDiagnostics(run: Record<string, unknown>): Record<string, unknown> {
  ...
  const resultingRevisionId = safeReference(run.resulting_revision_id) ?? safeReference(run.published_revision_id);
  ...
  if (resultingRevisionId !== null) {
    diagnosisSequence.push({
      code: "inspect_backup",
      method: "GET",
      path: `/v1/catalogue-revisions/${encodeURIComponent(resultingRevisionId)}/backups`,
    });
  }
  const retry =
    retryAvailable && runId !== null && !(evidenceBacked && state === "published")
  ...
  if (retry !== null) {
    diagnosisSequence.push({
      code: evidenceBacked ? "retry_evidence_collection" : "retry_ingestion_run",
      method: retry.method,
      path: retry.path,
    });
  }
  return {
    ...
    references: {
      ...
      expected_catalogue_revision_id: safeReference(run.expected_current_revision_id),
      resulting_catalogue_revision_id: resultingRevisionId,
      candidate_digest: candidateDigest,
      adapter_versions: adapterVersions,
      workflow: {
        status_path: runId === null ? null : `/v1/ingestion-runs/${encodeURIComponent(runId)}`,
      ...
      backup: {
        status_path:
          resultingRevisionId === null
            ? null
            : `/v1/catalogue-revisions/${encodeURIComponent(resultingRevisionId)}/backups`,
      },
      recovery: { status_path: "/v1/status" },
    },
    terminal_evidence: {
      state,
      terminal_at: safeReference(run.terminal_at) ?? safeReference(run.collection_completed_at),
      failure,
...
function retainedAdapterVersions(run: Record<string, unknown>): string[] {
  ...
  const versions = plans.flatMap((plan) => {
    ...
    const value = safeReference((plan as Record<string, unknown>).adapter_version);
...
function safeWorkflowReferences(value: unknown): Record<string, unknown> {
  ...
  return {
    ...
    child_ids: childIds,
    current_attempt_id: currentAttempt === null ? null : safeReference(currentAttempt.id),
...
function safeCoverage(run: Record<string, unknown>): Record<string, number> {
  const counts =
    run.evidence_counts !== null && typeof run.evidence_counts === "object" && !Array.isArray(run.evidence_counts)
  ...
  return {
    evidence_plan_count: Array.isArray(run.evidence_plans) ? run.evidence_plans.length : 0,
    source_snapshot_count: count("snapshot_count", run.snapshots),
    source_observation_set_count: count("observation_set_count", run.observation_sets),
