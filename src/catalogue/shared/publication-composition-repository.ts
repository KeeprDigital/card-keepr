...
/** Bounded handoff to backup/recovery: exact durable identities and registered game roots. */
export function publicationBackupReservationStatement(db: CatalogueStore, attemptId: string) {
  return repositoryStatements(db)
    .prepare(
      `SELECT b.idempotency_key AS backup_attempt_id,b.publication_operation_id,
