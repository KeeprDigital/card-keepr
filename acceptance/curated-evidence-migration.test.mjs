...
test("Curated retention migration protects acknowledged dependencies without rewriting history or tombstones", async (t) => {
  ...
  const migration = await readFile(new URL("0037_curated_evidence_retention.sql", root), "utf8");
  database.exec("BEGIN");
