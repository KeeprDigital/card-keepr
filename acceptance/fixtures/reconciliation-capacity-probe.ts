  ...
  async fetch(request: Request, env: Env) {
    if (new URL(request.url).pathname === "/setup") {
      ...
      const source = await injectFixtureEvidencePlan(env.CATALOGUE_DB, {
        ...
        requests: [
          { id: "cards", method: "GET", url: "https://official-source.invalid/reconciliation/scale-1001-products" },
        ],
      });
      ...
      const transport = {
        fetch: async (url: string) =>
          sourceUrl
            ? fetch(sourceUrl)
            : Response.json(reconciliationSourceDocument("scale-1001-products", "", String(url))),
      ...
      const response = await worker.fetch(
        new Request("https://owner.invalid/v1/game-candidates", {
          method: "POST",
          headers: { authorization: "Bearer probe-owner", "content-type": "application/json" },
          body: JSON.stringify({
            ingestion_run_id: source.id,
            supported_game: "one-piece",
            expected_game_revision_id: "catrev_spine_000",
            idempotency_key: "capacity-memory-candidate",
          }),
        }),
      ...
      const captured = await env.CATALOGUE_DB.prepare(
        ...
        .bind(source.id)
        .all<{ content_digest: string; content_byte_length: number }>();
      return Response.json({ candidate: await response.json(), params, captured_source: captured.results });
    }
    ...
    await runReconciliationWorkflow(
      env,
      { instanceId: "capacity-memory-root", payload: params } as WorkflowEvent<ReconciliationWorkflowParams>,
      {
            ...
            try {
              ...
              console.info(
                JSON.stringify({
                  ...
                  runtime: "probe-driver",
                  request: { method: "WORKFLOW", route: "/probe/reconciliation" },
                  workflow: { step: phase },
    ...
    const status = await worker.fetch(
      new Request(`https://owner.invalid/v1/game-candidates/${id}`, {
        headers: { authorization: "Bearer probe-owner" },
      }),
      env,
    );
