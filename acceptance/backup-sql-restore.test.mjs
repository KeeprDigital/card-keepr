...
test("a real SQL export restores a multi-Card catalogue whose FTS, API, and Curated provenance are verified", async (t) => {
  ...
  const { representative_product_digest: _legacyProductDigest, ...legacyProductOnlyExpected } = productOnlyExpected;
  await assert.doesNotReject(
    recovery.verifyRestoredCatalogue(database, {
      expectedRevisionId: "catrev_restore_acceptance",
      expectedSchemaMigrationLevel,
  ...
  restored.exec("DELETE FROM revision_products");
  const emptyExpected = await recovery.captureCatalogueVerificationEvidence(database, "catrev_restore_acceptance");
  await assert.rejects(
    recovery.verifyRestoredCatalogue(database, {
      expectedRevisionId: "catrev_restore_acceptance",
      expectedSchemaMigrationLevel,
...
function removeCardProjection(database) {
  database.exec(`
