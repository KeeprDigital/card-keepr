// [DEBUG-323-mixed-phase] Temporary, opt-in, metadata-only hosted diagnostic.
import { appendFileSync, mkdirSync, writeFileSync } from "node:fs";
import { join } from "node:path";

const directory = process.env.KEEPR_MIXED_PHASE_DIAGNOSTICS;
const slot = Number(process.env.KEEPR_MIXED_PHASE_SLOT ?? 0);
...
export function journeyStage(name) {
  if (!enabled || slot) return;
  if (lastStarted !== null) {
  ...
  record({ event: "journey-started", stage: name });
}
...
export function childPhaseEnvironment() {
  ...
  if (cliSlots > 32) return { KEEPR_MIXED_PHASE_DIAGNOSTICS: "", KEEPR_MIXED_PHASE_SLOT: "" };
  return { KEEPR_MIXED_PHASE_DIAGNOSTICS: directory, KEEPR_MIXED_PHASE_SLOT: String(cliSlots) };
}
