  ...
  test(`${migration.game} CHECK widening preserves populated ancestors, decisions and inbound foreign keys`, async () => {
    ...
    try {
      ...
      for (const name of (await readdir(root))
        .filter((n) => n.endsWith(".sql") && Number.parseInt(n, 10) <= migration.baseline)
        .sort())
      ...
      assert.deepEqual(queries.indexesAndViews(db).all(), projections);
      assert.deepEqual(schema.foreignKeyViolations(db).all(), []);
      assert.equal(schema.schemaMigrationLevel(db).get().migration_level, migration.level);
      queries.seedGameCheckpoint(db).run(`product_reduction:${migration.game}`, "f".repeat(64));
      queries.setPrintingGame(db).run(migration.game);
      assert.throws(() => db.exec("BEGIN;" + migrationSql), /malformed JSON/u);
      db.exec("ROLLBACK");
      assert.equal(schema.schemaMigrationLevel(db).get().migration_level, migration.level);
    } finally {
      db.close();
