...
test("the explicit Wrangler smoke tier covers every shipped lineage and the external CLI", async () => {
  assert.deepEqual(
  ...
  for (const { file } of smokeFlows) {
    await access(new URL(file, import.meta.url));
    assert.equal(isWranglerSmokeFile(`/repo/acceptance/${file}`), true);
  }
