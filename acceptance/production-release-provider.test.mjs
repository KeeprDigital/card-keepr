...
const preflightFailures = [
  ...
  ["missing secret", /worker_secret_inventory_mismatch/u, (url, document) => {
  ...
  ["unexpected live secret", /worker_secret_inventory_mismatch/u, (url, document) => {
    if (url.pathname.endsWith("/workers/scripts/card-keepr-api/settings")) document.result.bindings.push({ name: "CREDENTIAL_CONSUMER_PROOF_KEY", type: "secret_text" });
  }],
...
const versionEnvironment = (worker, tag = `release-2026-09-03-01-${worker === "card-keepr-api" ? "api" : "ingestion"}`) => ({
  ...environment(),
...
test("the uploaded version is verified against the release configuration before activation", async () => {
  for (const worker of target.worker_scripts) {
    ...
    assert.deepEqual(result, { worker, version_tag: versionEnvironment(worker).RELEASE_VERSION_TAG, version_id: `version-${worker}` });
  }
});

const versionFailures = [
  ["a var value", /uploaded_version_binding_mismatch/u, versionBinding("plain_text", (binding) => binding.text = "wrong")],
  ["D1 binding", /uploaded_version_binding_mismatch/u, versionBinding("d1", (binding) => binding.database_id = "wrong")],
  ["R2 binding", /uploaded_version_binding_mismatch/u, versionBinding("r2_bucket", (binding) => binding.bucket_name = "wrong")],
  ["service binding", /uploaded_version_binding_mismatch/u, versionBinding("service", (binding) => binding.entrypoint = "WrongEntrypoint")],
  ["Workflow binding", /uploaded_version_binding_mismatch/u, versionBinding("workflow", (binding) => binding.class_name = "WrongWorkflow")],
  ["rate-limit settings", /uploaded_version_binding_mismatch/u, versionBinding("ratelimit", (binding) => binding.simple.limit += 1)],
  ["a secret the version lost", /uploaded_version_binding_mismatch/u, (url, document) => {
    if (/\/versions\/version-/u.test(url.pathname)) document.result.resources.bindings = document.result.resources.bindings.filter((binding) => binding.type !== "secret_text");
  }],
  ["an unexpected privileged binding", /unexpected_worker_binding_type/u, (url, document) => {
    if (/\/versions\/version-/u.test(url.pathname)) document.result.resources.bindings.push({ name: "UNEXPECTED_KV", type: "kv_namespace", namespace_id: "privileged" });
  }],
  ["a missing tag", /release_version_not_found/u, (url, document) => {
    if (url.pathname.endsWith("/versions")) document.result.items = [];
  }],
  ["an ambiguous tag", /release_version_ambiguous/u, (url, document) => {
    if (url.pathname.endsWith("/versions")) document.result.items.push({ ...document.result.items[0], id: "version-duplicate" });
  }],
  ["malformed version resources", /malformed_worker_version/u, (url, document) => {
...
async function providerFetch(mutate = () => {}) {
  ...
  return async (input, init) => {
    ...
    if (worker) result = { bindings: bindings(configs[worker], worker) };
      ...
      result = { items: [
        { id: `version-${versionsOf}`, metadata: { created_on: "2026-09-03T03:00:00Z" }, annotations: { "workers/tag": tag, "workers/message": "release" } },
        { id: "version-older", metadata: { created_on: "2026-09-01T00:00:00Z" }, annotations: { "workers/tag": "release-2026-09-01-01-old" } },
      ] };
    } else if (versionOf) {
      result = { id: `version-${versionOf[2]}`, resources: { bindings: bindings(configs[versionOf[1]], versionOf[1]), script_runtime: { compatibility_date: "2026-07-29" } } };
    } else if (databaseId) {
