...
test("black-box smoke covers auth, representative reads, retained exports, and stale cursors", async () => {
  ...
  const result = await runProductionSmoke(
    ...
    async (url, init) => {
      ...
      const fixture = revisions.find((item) =>
        [item.card_cursor, item.search_cursor, item.printing_cursor].includes(after),
      );
      const revision =
        fixture?.revision_id ??
        (parsed.pathname.includes("catrev-previous")
          ? "catrev-previous"
          : parsed.pathname.includes("catrev-old")
            ? "catrev-old"
            : "catrev-current");
      const headers = { "content-type": "application/json", "x-catalogue-revision": revision };
      if (after === staleCursor) {
        return new Response(JSON.stringify({ code: "cursor_revision_unavailable" }), {
          status: 409,
          headers: { "content-type": "application/problem+json" },
        });
      }
      const body =
        healthDocument(parsed.pathname) ??
        (parsed.pathname === "/v1/catalogue"
  ...
  assert.ok(visited.every((path) => !path.includes("legality")));
...
test("black-box smoke appends route paths to an API base that carries a mount path", async () => {
  ...
  await runProductionSmoke(
    ...
    async (url, init) => {
      ...
      const body =
        healthDocument(parsed.pathname) ??
        (parsed.pathname === "/api/v1/catalogue"
          ? { meta: { catalogue_revision_id: "catrev-current" } }
...
test("black-box smoke rejects missing retained or stale fixtures before traffic", async () => {
  ...
  await assert.rejects(
    runProductionSmoke({ apiUrl: "https://api.example.invalid", apiKey: "key", revisions: [] }, async () => {
      calls += 1;
      return new Response();
    }),
  ...
  const result = await runBootstrapSmoke(
    ...
    async (url, init) => {
      ...
      if (parsed.pathname === "/api/v1/catalogue") {
        return new Response(JSON.stringify({ meta: { catalogue_revision_id: "catrev_spine_000" } }), {
          status: 200,
    ...
    runBootstrapSmoke(
      ...
      async (url, init) => {
        ...
        if (init.headers.authorization !== "Bearer traffic-key")
          return new Response(JSON.stringify({ code: "authentication_required" }), { status: 401 });
        if (parsed.pathname === "/api/v1/catalogue") {
          return new Response(JSON.stringify({ meta: { catalogue_revision_id: "catrev_first" } }), {
            status: 200,
            headers: { "x-catalogue-revision": "catrev_first" },
          });
        }
        return new Response(JSON.stringify(healthDocument(parsed.pathname) ?? {}), { status: 200 });
      },
    ...
    runBootstrapSmoke(
      {
        ...
        apiKey: "key",
        ingestionUrl: "https://card.keepr.digital/ingest",
        currentRevisionId: "catrev_first",
      },
      async () => {
        calls += 1;
        return new Response();
