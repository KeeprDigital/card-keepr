...
release: account, Worker scripts, catalogue and separate disposable verification
D1 databases, private evidence/image/export/backup buckets and Workflow bindings.
Replace placeholder account/D1 IDs and keep the configured database ID consistent
with its binding. R2 buckets remain private without r2.dev or public custom domains.

Install `API_BEARER_KEY` and its replacement only on the API Worker;
...
The production environment's branch policy and exact owner confirmation also apply.

`migrations/` is one schema baseline (`0001_baseline.sql`, schema level 1,
see [definition policy](../architecture.md#definition-changes-and-go-live)) followed by guarded forward migrations. An empty database is
...
Cloudflare Health Check watches (see [maintenance](maintenance.md#health-and-unexpected-failures)).
...
Track deployment and final freeze acceptance in [issue #239](https://github.com/KeeprDigital/card-keepr/issues/239).
...
Local SQLite and provider-double tests establish protocol behavior only. At live
execution retain the exact SHA, source/destination IDs, dispatch digest, both
observed version IDs, routes, terminal phase evidence and verified recovery IDs.
Final baseline equivalence, deployed protocol verification, destination catalogue
regeneration and explicit Go-Live acceptance remain separate requirements.
