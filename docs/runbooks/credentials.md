...
Cloudflare API with one bootstrap token. Cloudflare keeps two token stores, the
owner's user-owned tokens (My Profile) and the account-owned tokens (Manage
Account); the script reads and writes both and labels every row with its owner.
Create the bootstrap token under My Profile from the **Create additional tokens**
template (User > API Tokens > Edit, the only template that offers it) and add
Account > Account API Tokens > Edit for this account, give it a one-day TTL,
export it as `KEEPR_TOKEN_ADMIN` in the owner shell, and revoke it with the
script when the work is done: it can mint tokens with any of the owner's
permissions, so it never outlives the task. A store it cannot read is reported
as a warning. Every command is a dry run until `--apply` is given.
...
name with the grant from the tables below, in the store of the token it replaces
(`--owner=user|account` decides when nothing is replaced or both stores hold one), runs the [probe](#probe) with the new
