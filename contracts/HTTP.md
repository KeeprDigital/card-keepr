...
workflow separately from owner-key administration. The dormant promotion route
(`POST /v1/production-promotions`, production only; nothing calls it) accepts the
same run's production-environment identity. Production forwards that identity to
staging's read-only `POST /v1/staging-deployments/{release}/promotion-outcome`; see
the [release procedure](../docs/runbooks/production-release.md#dormant-promotion-endpoint).
